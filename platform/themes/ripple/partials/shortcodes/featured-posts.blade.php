                        ...
                        <div class="post-group__left">
                            <article class="post post__inside post__inside--feature">
                                <div class="post__thumbnail">
                                    <img src="{{ RvMedia::getImageUrl($featureItem->image, 'featured', false, RvMedia::getDefaultImage()) }}" alt="{{ $featureItem->name }}" loading="lazy"><a href="{{ $featureItem->url }}" title="{{ $featureItem->name }}" class="post__overlay"></a>
                                </div>
                            ...
                            @else
                                <div class="post-group__item">
                                    <article class="post post__inside post__inside--feature post__inside--feature-small">
                                        <div class="post__thumbnail"><img src="{{ RvMedia::getImageUrl($featureItem->image, 'medium', false, RvMedia::getDefaultImage()) }}" alt="{{ $featureItem->name }}" loading="lazy"><a href="{{ $featureItem->url }}" title="{{ $featureItem->name }}" class="post__overlay"></a></div>
                                        <header class="post__header">
                                            <h3 class="post__title"><a href="{{ $featureItem->url }}">{{ $featureItem->name }}</a></h3>
                                        </header>
                                    </article>
